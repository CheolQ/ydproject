...
router.get('/getbestproduct', async (req, res) => {
    let result = await query('bestProductList');
    res.send(result);
});

router.get('/products', async (req, res) => {
    const category = req.query.category || '';
    let result = await query('mainCategoryProductList', [category]);
    res.send(result);
});
